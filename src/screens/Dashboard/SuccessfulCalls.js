...
import { fetchSuccessfulCalls, recordCall } from './store';

const SuccessfulCalls = ({ navigation }) => {
  ...
  const handleSaveOutcome = async (callData) => {
    ...
    try {
      ...
      hasShownModal.current = false;
      // Refresh the successful calls list
      dispatch(fetchSuccessfulCalls({ page: 1, size: 20 }));
    } catch (error) {
  ...
  const renderCallItem = ({ item }) => {
    const transformedItem = {
      ...
      callCount: 1, // Successful calls show as single entry
      status: item.status,
      notes: item.notes,
  ...
  return (
    <View style={[C.bgWhite, L.f1]}>
      <Header navigation={navigation} label_center='Successful Calls' showDrawer={true} />
      
      {responseDataDashBoard?.isLoading && <Loader />}
      
      <FlatList
        data={responseDataDashBoard?.successfulCalls || []}
          ...
          {selectedCall && (
            <>
              ...
              {selectedCall.scheduledFor && (
                <Text style={[F.fsOne4, C.fcGray, F.ffM, L.mB5]}>Follow-up: {new Date(selectedCall.scheduledFor).toLocaleString()}</Text>
              )}
            </>
          )}
